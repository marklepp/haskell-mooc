...
totalPrice (MkShoppingEntry _ price amount) = 
  price * (fromIntegral amount)
...
buyOneMore (MkShoppingEntry item price amount) = 
  MkShoppingEntry item price (amount + 1)
...
getName (Person _age name) = name

-- getAge returns the age of the person
getAge :: Person -> Int
getAge (Person age _name) = age

-- setName takes a person and returns a new person with the name changed
setName :: String -> Person -> Person
setName name (Person age _name) = Person age name

-- setAge does likewise for age
setAge :: Int -> Person -> Person
setAge age (Person _age name) = Person age name
...
fromBin :: Bin -> Int
fromBin b = fromBin' b 0 0

fromBin' :: Bin -> Int -> Int -> Int
fromBin' End ex acc = acc
fromBin' (O b) ex acc = fromBin' b (ex + 1) acc
fromBin' (I b) ex acc = fromBin' b (ex + 1) (2^ex + acc)

toBin :: Int -> Bin
toBin n = foldl toBin' End binaryString
  where 
    binaryString :: String
    binaryString = printf "%b" n
    toBin' b '0' = O b
    toBin' b '1' = I b
