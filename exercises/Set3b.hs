...
sums' :: Int -> [Int] -> [Int]
sums' 0 all = all
sums' i all = sums' (i - 1) (i:[i + x | x <- all])
...
indexDefault (first:_) 0 _ = first 
indexDefault [] i def | i >= 0 = def
...
sorted [] = True
sorted (first:[]) = True
sorted (first:second:xs) 
  | first <= second = sorted (second:xs)
  | otherwise = False
...
sumsOf [] = []
sumsOf (first:[]) = [first]
sumsOf (first:second:rest) = first : (sumsOf (first + second : rest))
...
maybeMap _ [] = []
maybeMap f (x:xs) = maybeMap' (f x) (maybeMap f xs)

maybeMap' :: Maybe b -> [b] -> [b]
maybeMap' Nothing ys = ys
maybeMap' (Just y) ys = y:ys 
