...
firstHalf str 
  | strlen `mod` 2 == 1 = take (strlen `div` 2 + 1) str
  | otherwise = take (strlen `div` 2) str
  where strlen = length str

palindrome :: String -> Bool
palindrome str = (firstHalf str) == (firstHalf.reverse $ str)
...
capitalizeFirst "" = ""
capitalizeFirst (first:tail) = toUpper first : tail 
...
whileRight' check (Left x) = x
whileRight' check (Right x) = whileRight check x
...
joinToLength len strings = 
  filter (\x -> length x == len) 
         [first ++ second | first <- strings, second <- strings]
...
sumRights values = sum (rights values)
-- challenge: 
-- sumRights values = sum . map (either zero id) $ values
--   where zero _ = 0 
...
multiCompose [] a = a
multiCompose (func:rest) a = multiCompose' func rest a

multiCompose' :: (a -> a) -> [(a -> a)] -> a -> a
multiCompose' comp [] a = comp a
multiCompose' comp (func:rest) a = multiCompose' (comp . func) rest a
...
interpreter' [] results _ _ = reverse results
interpreter' (command:rest) results x y =
  case command of
    "up" -> next x (y + 1)
    "down" -> next x (y - 1)
    "left" -> next (x - 1) y
    "right" -> next (x + 1) y
    "printX" -> continue (show x : results) x y
    "printY" -> continue (show y : results) x y
    _ -> continue ("invalid command" : results) x y
  where next = interpreter' rest results
        continue = interpreter' rest
