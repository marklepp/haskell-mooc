...
treeSize :: Tree a -> Int
treeSize Empty = 0
treeSize (Node _ ta tb) = 1 + (treeSize ta) + (treeSize tb)
...
treeMax Empty = 0
treeMax (Node val ta tb) = max val $ max (treeMax ta) (treeMax tb)
...
allValues condition (Node value ta tb) = 
  condition value && 
  allValues condition ta && 
  allValues condition tb
...
mapTree f (Node val ta tb) = Node (f val) (mapTree f ta) (mapTree f tb)
...
cull val (Node other ta tb)
  | val == other = Empty
  | otherwise = Node other (cull val ta) (cull val tb)
...
walk (StepL:rest) (Node _a ta _tb) = walk rest ta 
walk (StepR:rest) (Node _a _ta tb) = walk rest tb 
...
set [] val (Node _a ta tb) = Node val ta tb
set (StepL:rest) val (Node a ta tb) = Node a (set rest val ta) tb 
set (StepR:rest) val (Node a ta tb) = Node a ta (set rest val tb)
...
search' _val Empty _steps = Nothing
search' val (Node other ta tb) steps 
  | val == other = Just (reverse steps)
  | otherwise = 
    let foundA = search' val ta (StepL:steps) in
      case foundA of 
          Just a -> Just a
          Nothing -> 
            let foundB =  search' val tb (StepR:steps) in
            case foundB of
                 Just a -> Just a
                 Nothing -> Nothing
