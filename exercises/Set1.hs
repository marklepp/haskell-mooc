...
sumTo num = sumTo' num 0 1

sumTo' :: Integer -> Integer -> Integer -> Integer
sumTo' num sum current = if num < current then sum
                        else sumTo' num (sum + current) (current + 1)
...
power num toPower
--  | toPower < 0  = 1 / (power num (-toPower))
  | toPower == 0 = 1
  | otherwise = num * (power num (toPower - 1))
...
ilog3 num = ilog3' num 0

ilog3' :: Integer -> Integer -> Integer
ilog3' 0 times = times
ilog3' num times = ilog3' (num `div` 3) (times + 1)
