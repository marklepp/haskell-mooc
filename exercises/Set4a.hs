...
rangeOf a = let b = sort a in (last b) - (head b)
...
longest xss = head 
              ...
              . filter bymaxlength $ xss
              where bymaxlength xs = length xs == maxlength
                    maxlength = foldl max 0 (map length xss)
...
incrementKey k [] = []
incrementKey k ((key,value):cdr)
  | k == key = (key, value + 1) : rest
  | otherwise = (key, value) : rest
  where rest = incrementKey k cdr
...
freqs lst = foldr incrementCount Map.empty lst
  where incrementCount x counts = Map.alter increment x counts
...
transfer from to amount bank 
  ...
  | otherwise = Map.adjust (subtract amount) from 
...
maxIndex arr = 
  let assoclist = (Data.Array.assocs arr)
      maxValue (i1, val1) (i2, val2) 
        | val1 > val2 = (i1, val1)
        | otherwise = (i2, val2)
  in fst (foldr maxValue (assoclist !! 0) assoclist)
