...
instance Eq a => Eq (List a) where
  Empty == Empty = True
  (LNode a resta) == (LNode b restb) = a == b && resta == restb
  _ == _ = False
...
instance Eq RationalNumber where
  (RationalNumber numera denoma) == (RationalNumber numerb denomb) = 
    numera * denomb == numerb * denoma
...
simplify (RationalNumber numer denom) = 
  let common = gcd numer denom
  in RationalNumber (numer `div` common) (denom `div` common)
