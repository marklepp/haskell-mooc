...
blendColor (Color r1 g1 b1) (Color r2 g2 b2) = 
  Color (ave r1 r2) (ave g1 g2) (ave b1 b2) where ave x1 x2 = (x1 + x2) `div` 2 

combine :: (Color -> Color -> Color) -> Picture -> Picture -> Picture
combine blender (Picture f1) (Picture f2) = Picture f
  where f coord = blender (f1 coord) (f2 coord)
...
union (Shape f1) (Shape f2)= Shape uni
  where uni coord = f1 coord || f2 coord

cut :: Shape -> Shape -> Shape
cut (Shape f1) (Shape f2) = Shape cutter
  where cutter coord = f1 coord && not (f2 coord)
...
instance Transform Fill where
  apply (Fill color) _p = Picture f where f _ = color

data Zoom = Zoom Int
  deriving Show

instance Transform Zoom where
  apply (Zoom scale) (Picture p) = Picture scaledUp
    where scaledUp (Coord x y) = p (Coord (div x scale) (div y scale))

data Flip = FlipX | FlipY | FlipXY
  deriving Show

instance Transform Flip where
  apply FlipX (Picture pic) = Picture flipped
    where flipped (Coord x y) = pic (Coord (-x) y)

  apply FlipY (Picture pic) = Picture flipped
    where flipped (Coord x y) = pic (Coord x (-y))

  apply FlipXY (Picture pic) = Picture flipped
    where flipped (Coord x y) = pic (Coord y x)
...
instance Transform Blur where
  apply Blur (Picture pic) = Picture f
    where f (Coord x y) =
            averageColors (map pic [Coord (x - 1) y,
                                    Coord x y, 
                                    Coord (x + 1) y, 
                                    Coord x (y - 1), 
                                    Coord x (y + 1)])
...
averageColors colors = Color r g b
  where 
    r = average (map getRed colors)
    g = average (map getGreen colors)
    b = average (map getBlue colors)

average :: [Int] -> Int
average xs = average' xs 0 0

average' :: [Int] -> Int -> Int -> Int
average' [] sum len = sum `div` len
average' (x:rest) sum len = average' rest (sum + x) (len + 1)
...
instance Transform BlurMany where
  ...
  apply (BlurMany times) pic = apply (BlurMany (times - 1)) (apply Blur pic)
